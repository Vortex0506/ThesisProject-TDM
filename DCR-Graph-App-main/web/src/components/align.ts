...
export default (trace: Trace, graph: LabelDCRPP, costFun: CostFun = alignCost, toDepth: number = Infinity): Alignment => {
  ...
  const alignTrace = (
    ...
    for (const event of enabled) {
      const alignment = newGraphEnv(graph, () => {
        execute(event, graph);
        return alignTrace(trace, graph, curCost + alignCost("model-skip", event), ++curDepth);
      });
      if (alignment.cost < bestAlignment.cost) {
        alignment.trace.unshift(event);
        maxCost = alignment.cost;
  ...
  //console.log(toDepth);
  maxCost = toDepth !== Infinity ? toDepth : trace.map(event => costFun("trace-skip", event)).reduce((acc, cur) => acc + cur, 0) + alignTrace([], graph).cost;
  //console.log(maxCost);
  ...
  return alignTrace(trace, graph, 0);
};